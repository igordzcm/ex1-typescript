...
function addCrewMember(name: string, nacionalidade: string, spaceship: {name: string, crewLimit: number, crew: object[]}){
    const crewMember = {
        name,
        nacionalidade
    }
...
function sendOnMission(spaceship: {name: string, crew: object[], crewLimit: number, inMission: boolean}){
...
function fourthMenuOption() {
    ...
    spaceships.forEach((spaceship: {
      ...
      pilot: string,
      crewLimit: number,
      crew: {name: string, nacionalidade: string}[],
      inMission: boolean
    }) => {
      list += `
        Nave: ${spaceship.name}
